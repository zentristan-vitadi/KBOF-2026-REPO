...
function displayMatches(umkm, tenagaKerja) {
  ...
  const setUmkmName = 'Siti Ruhayah Homestay';
  const setUMKMlocation = 'Bogor';
  const setUmkmAvatar = 'S';
  const setUmkmCategory = 'Homestay';
  const setUmkmDesc = 'Serving comfortable homestay';
  const setUmkmSkills = ['Hospitality', 'Cleaning', 'Service'];

  grid.innerHTML = `
                <div class="match-card">
                    <div class="match-card-type">${umkm.type}</div>
                    <div class="match-card-avatar">${setUmkmAvatar}</div>
                    <h4>${setUmkmName}</h4>
                    <div class="match-card-info">📍 ${setUMKMlocation}</div>
                    <div class="match-card-info">⭐ ${umkm.rating} Rating</div>
                    <div class="match-card-info">🏪 ${setUmkmCategory}</div>
                    <div class="match-card-desc">${setUmkmDesc}</div>
                    <div class="match-card-skills">
                        ${setUmkmSkills.map(skill => `<span class="skill-tag">${skill}</span>`).join('')}
                    </div>
                </div>
